        ...
        <div class="row">
			<div class="col-md-3">
				<div class="form-group">
					...
					<?php echo $this->Form->input('transaction_date', ['type' => 'text','label' => false,'class' => 'form-control input-sm date-picker','data-date-format' => 'dd-mm-yyyy','value' => date("d-m-Y",strtotime($payment->transaction_date)),'data-date-start-date' => $start_date,'data-date-end-date' => $end_date]); ?>
				</div>
		...
		<table width="100%" id="main_table">
			...
			<?php  foreach($payment->payment_rows as $payment_row){ ?> 
				<tr class="main_tr" old_received_from_id="<?php echo $payment_row->received_from_id; ?>">
					<td><?php echo $this->Form->input('received_from_id', ['empty'=>'--Select-','options'=>$receivedFroms,'label' => false,'class' => 'form-control input-sm received_from','value'=>$payment_row->received_from_id]); ?>
					<div class="show_grns">
					...
					elseif($payment_row->received_from_id=='105' || $payment_row->received_from_id=="168" || $payment_row->received_from_id=='316'){
							...
							foreach($invoice as $invoice1)
								{ 
									$invoiceIds = explode(',',$payment_row->invoice_ids);
